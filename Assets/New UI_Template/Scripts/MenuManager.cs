...
namespace LevelManagement
{
    public class MenuManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private Dungeon1_LevelMenu dungeon1_LevelMenu;
        [SerializeField]
        ...
        public IEnumerator CloseMenuWithAnimation(float animationDuration)
        {
            if (_menuStack.Count == 0)
            {
                Debug.LogWarning("MENUMANAGER CloseMenu ERROR: No menus in stack!");
        ...
        public void OpenMenuWithAnimationPart1(Menu menuInstance)
        {
            if (menuInstance == null)
            {
                Debug.Log("MENUMANAGER OpenMenu ERROR: invalid menu");
        ...
        public void OpenMenuWithAnimationPart2(Menu menuInstance)
        {
            if (_menuStack.Count > 0)
            {
                foreach (Menu menu in _menuStack)
                {
                    if (menu.Equals(menuInstance)) { continue; }
                    menu.gameObject.SetActive(false);
                }
            }
        }
