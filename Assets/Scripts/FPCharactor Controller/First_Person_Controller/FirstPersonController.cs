    ...
    public class FirstPersonController : MonoBehaviour
    {
        #region Variables
            #region Private Serialized     
                ...
                #region Sprint Energy Settings
                    PlayerSprintSystem playerSprint;
                    public bool CanRunPlayer => CanRun();
                    public bool HitWall => m_hitWall;
                #endregion
                ...
                #region Audio Data
                    [Space, Header("Audio Data")]
                    [SerializeField] private SoundBase playerFootSound;
                    [SerializeField] private SoundBase playerJumpSound;
                    [SerializeField] private SoundBase playerLandingSound;
                    public bool IsFootSoundPlayed => m_isFootAudioPlayed;
                    public bool IsJumpSoundPlayed => m_isJumpAudioPlayed;
                    #endregion
                    [Space]
        ...
        private CharacterController m_characterController;
                    ...
                    private bool m_isFootAudioPlayed;
                    private bool m_isJumpAudioPlayed;
                ...
                #region Debug
                    ...
                    [Space]
                    [SerializeField] private Vector3 m_finalMoveDir;
                    ...
                    [SerializeField] private float m_finalSmoothCurrentSpeed;
                    [SerializeField] private float m_walkRunSpeedDifference;
                    
                    [Space]
                    [SerializeField] private float m_finalRayLength;
                    [SerializeField] private bool m_hitWall;
                    [SerializeField] private bool m_isGrounded;
                    [SerializeField] private bool m_previouslyGrounded;
                    
                    [Space]
                    [SerializeField] private float m_initHeight;
                    [SerializeField] private float m_crouchHeight;
                    [SerializeField] private Vector3 m_initCenter;
                    [SerializeField] private Vector3 m_crouchCenter;

                    [Space]
                    [SerializeField] private float m_initCamHeight;
                    [SerializeField] private float m_crouchCamHeight;
                    [SerializeField] private float m_crouchStandHeightDifference;
                    [SerializeField] private bool m_duringCrouchAnimation;
                    [SerializeField] private bool m_duringRunAnimation;

                    [Space]
                    [SerializeField] private float m_inAirTimer;

                    [Space]
                    [SerializeField] private float m_inputVectorMagnitude;
                    [SerializeField] private float m_smoothInputVectorMagnitude;
                #endregion
            #endregion
        ...
        #region Custom Methods
            #region Initialize Methods    
                protected virtual void GetComponents()
                {
                    ...
                    playerSprint = GetComponent<PlayerSprintSystem>();
                ...
                {   
                    ...
                    m_crouchCenter = (m_crouchHeight / 2f + m_characterController.skinWidth) * Vector3.up + new Vector3(0, -0.1f, 0f);
            ...
            #region Smoothing Methods
                ...
                {
                    m_inputVector = PlayerMovement_InputData.Instance.InputVector.normalized;
                ...
                protected virtual void SmoothSpeed()
                {
                    ...
                    if(PlayerMovement_InputData.Instance.IsRunning && CanRun())
                    {
            ...
            #region Locomotion Calculation Methods
                ...
                protected virtual void CheckIfWall()
                {
                    ...
                    if(PlayerMovement_InputData.Instance.HasInput && m_finalMoveDir.sqrMagnitude > 0)
                ...
                protected virtual bool CanRun()
                {
                    ...
                    return _dot >= canRunThreshold && !PlayerMovement_InputData.Instance.IsCrouching ? true : false && playerSprint.CurrentEnergy > playerSprint.MinEnergyToRun();
                }
                ...
                {
                    m_currentSpeed = PlayerMovement_InputData.Instance.IsRunning && CanRun() ? runSpeed : walkSpeed;
                    m_currentSpeed = PlayerMovement_InputData.Instance.IsCrouching ? crouchSpeed : m_currentSpeed;
                    m_currentSpeed = !PlayerMovement_InputData.Instance.HasInput ? 0f : m_currentSpeed;
                    m_currentSpeed = PlayerMovement_InputData.Instance.InputVector.y == -1 ? m_currentSpeed * moveBackwardsSpeedPercent : m_currentSpeed;
                    m_currentSpeed = PlayerMovement_InputData.Instance.InputVector.x != 0 && PlayerMovement_InputData.Instance.InputVector.y ==  0 ? m_currentSpeed * moveSideSpeedPercent :  m_currentSpeed;
                }
            ...
            #region Crouching Methods
                ...
                {
                    if(PlayerMovement_InputData.Instance.CrouchClicked && m_isGrounded)
                        InvokeCrouchRoutine();
                }

                protected virtual void InvokeCrouchRoutine()
                {
                    if(PlayerMovement_InputData.Instance.IsCrouching)
                        if(CheckIfRoof())
                            return;
                ...
                protected virtual IEnumerator CrouchRoutine()
                {
                    ...
                    float _desiredHeight = PlayerMovement_InputData.Instance.IsCrouching ? m_initHeight : m_crouchHeight;
                    Vector3 _desiredCenter = PlayerMovement_InputData.Instance.IsCrouching ? m_initCenter : m_crouchCenter;

                    Vector3 _camPos = m_yawTransform.localPosition;
                    float _camCurrentHeight = _camPos.y;
                    float _camDesiredHeight = PlayerMovement_InputData.Instance.IsCrouching ? m_initCamHeight : m_crouchCamHeight;

                    PlayerMovement_InputData.Instance.IsCrouching = !PlayerMovement_InputData.Instance.IsCrouching;
                    m_headBob.CurrentStateHeight = PlayerMovement_InputData.Instance.IsCrouching ? m_crouchCamHeight : m_initCamHeight;
        ...
        #region Locomotion Apply Methods
                ...
                protected virtual void HandleHeadBob()
                {
                    ...
                    if(PlayerMovement_InputData.Instance.HasInput && m_isGrounded  && !m_hitWall)
                    {
                        if(!m_duringCrouchAnimation) // we want to make our head bob only if we are moving and not during crouch routine
                        {
                            m_headBob.ScrollHeadBob(PlayerMovement_InputData.Instance.IsRunning && CanRun(), PlayerMovement_InputData.Instance.IsCrouching, PlayerMovement_InputData.Instance.InputVector);
                ...
                {
                    m_cameraController.HandleSway(m_smoothInputVector, PlayerMovement_InputData.Instance.InputVector.x);
                }
                ...
                protected virtual void HandleRunFOV()
                {
                    if(PlayerMovement_InputData.Instance.HasInput && m_isGrounded  && !m_hitWall)
                    {
                        if(PlayerMovement_InputData.Instance.RunClicked && CanRun())
                        {
                            m_duringRunAnimation = true;
                            m_cameraController.ChangeRunFOV(false);
                        }

                        if(PlayerMovement_InputData.Instance.IsRunning && CanRun() && !m_duringRunAnimation )
                        {
                            m_duringRunAnimation = true;
                            m_cameraController.ChangeRunFOV(false);
                        }
                    ...
                    if(PlayerMovement_InputData.Instance.RunReleased || !CanRun() || !PlayerMovement_InputData.Instance.HasInput || m_hitWall)
                    {
                ...
                {
                    if(PlayerMovement_InputData.Instance.JumpClicked && !PlayerMovement_InputData.Instance.IsCrouching)
                    {
                ...
                {
                    if (CanRun() && PlayerMovement_InputData.Instance.HasInput && !m_hitWall && PlayerMovement_InputData.Instance.IsRunning)
                    {
                        playerSprint.canTakeEnergy = true;
                    }
                    else
                    {
                        playerSprint.canTakeEnergy = false;
                    }
                }
                ...
                protected virtual void ApplySounds()
                {
                    if(m_characterController.isGrounded && m_characterController.velocity.magnitude > 1f && !m_isFootAudioPlayed && !PlayerMovement_InputData.Instance.IsCrouching)
                    {
                        playerFootSound.Play(m_source);
                        m_isFootAudioPlayed = true;
                        StartCoroutine(FootSoundResetRoutine());
                    }
                    else if(m_characterController.isGrounded && PlayerMovement_InputData.Instance.JumpClicked && !PlayerMovement_InputData.Instance.IsCrouching && !m_isJumpAudioPlayed)
                    {
                        playerJumpSound.Play(m_source);
                        m_isJumpAudioPlayed = true;
                        StartCoroutine(m_JumpSoundResetRoutine());
                    }
                }
                
                IEnumerator FootSoundResetRoutine()
                {
                    float length = m_source.clip.length * 2f;
                    float clipSpeed = PlayerMovement_InputData.Instance.IsRunning && CanRun() ? length / 2f : length;
                    yield return new WaitForSeconds(clipSpeed);
                    m_isFootAudioPlayed = false;
                }
                IEnumerator m_JumpSoundResetRoutine()
                {
                   yield return new WaitForSeconds(0.2f);
                   playerLandingSound.Play(m_source);
                   m_isJumpAudioPlayed = false;
                }
